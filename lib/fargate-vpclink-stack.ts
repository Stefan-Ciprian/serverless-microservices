...
export class FargateVpclinkStack extends cdk.Stack {
  ...
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    const csvServiceTaskDefinition = new ecs.FargateTaskDefinition(
      this,
      "csvServiceTaskDef",
      {
        memoryLimitMiB: 512,
    ...
    const csvServiceLogGroup = new logs.LogGroup(this, "csvServiceLogGroup", {
      logGroupName: "/ecs/CsvService",
      removalPolicy: cdk.RemovalPolicy.DESTROY,
    });
    ...
    const csvServiceLogDriver = new ecs.AwsLogDriver({
      logGroup: csvServiceLogGroup,
      streamPrefix: "CsvService",
    });
    ...
    const csvservicerepo = ecr.Repository.fromRepositoryName(
      this,
      "csvservice",
      "csv-service"
    );
    ...
    const csvServiceContainer = csvServiceTaskDefinition.addContainer(
      "csvServiceContainer",
      {
        image: ecs.ContainerImage.fromEcrRepository(csvservicerepo),
        logging: csvServiceLogDriver,
      }
    );
    ...
    csvServiceContainer.addPortMappings({
      containerPort: 80,
    });
    ...
    const csvServiceSecGrp = new ec2.SecurityGroup(
      this,
      "csvServiceSecurityGroup",
      {
        allowAllOutbound: true,
        securityGroupName: "csvServiceSecurityGroup",
        vpc: vpc,
      }
    );

    csvServiceSecGrp.connections.allowFromAnyIpv4(ec2.Port.tcp(80));
    ...
    const csvService = new ecs.FargateService(this, "csvService", {
      cluster: cluster,
      taskDefinition: csvServiceTaskDefinition,
      assignPublicIp: false,
      desiredCount: 2,
      securityGroup: csvServiceSecGrp,
      cloudMapOptions: {
        name: "csvService",
        cloudMapNamespace: dnsNamespace,
      },
    ...
    const csvServiceTargetGroup = this.httpApiListener.addTargets(
      "csvServiceTargetGroup",
      {
        port: 80,
        priority: 1,
        healthCheck: {
